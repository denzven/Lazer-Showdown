...
import { 
  ...
  Cpu, Grid, Trash2, Download, Wand2
} from 'lucide-react';
...
const LLM_PROMPT_CONTENT = `**Role:** Expert AI Architect and Game Theory Specialist

**Task:** Design a highly modular, foundational bot in JavaScript for a discrete-state stochastic board game called 'Lazer Showdown'. Rather than forcing a single rigid playstyle, this bot should serve as an experimental sandbox for computer engineering students to explore various game-theoretic architectures, heuristic utility functions, and distinct AI "personalities" (e.g., strictly mathematical/risk-averse vs. chaotic/risk-seeking).

**Game Theoretic Overview:**
* The environment is a deterministic 8x8 grid topology featuring fixed combinatorial elements (mirrors).
* It operates as an asymmetric zero-sum game with heterogeneous agents: an attacking laser piece and three defending utility nodes (valued at 20, 30, and 50 points).
* **Stochastic Elements:** General state transitions are governed by a 2d6 dice framework. Additionally, there is a distinct "challenge" mechanic that resolves via a coin toss by one player and a simultaneous 1d6 roll by both players, introducing localized probabilistic variance that must be accounted for in expected value (EV) calculations.

**System Architecture & Strategy Requirements:**
Your bot must adhere to the asynchronous Strategy Interface by implementing and exporting three essential lifecycle handlers. Provide creative, experimental approaches that students can toggle or expand upon:

1. **\`getSetupAction(board, phase, playerColor, challengedPiece)\`**
Engineer a dynamic initialization script that allows for multiple topological strategies. 
* Provide logic for a "Cautious" personality that maximizes the topological safety of defending pieces by placing them on low-probability threat vectors away from grid edges.
* Provide an alternative "Aggressive" logic that locks the laser entity into optimal initial firing quadrants to maximize the initial branching factor of attack vectors before resolving with a "confirm-setup" token.

2. **\`getPlayAction(board, role, actionPoints, gameState, botPlayer)\`**
Build a versatile state-space exploration foundation that isn't locked into a single paradigm. 
* Iterate through the legal action space generated by \`getPossibleActions(board, role)\`.
* Simulate future Markov states in a decoupled, thread-safe sandbox using \`applyLightweightAction(board, action)\`.
* Implement branching utility functions that allow the bot to exhibit different behaviors. For instance, combine deterministic Expectiminimax search patterns with experimental heuristics. 
* **Attacker Utility:** Use \`traceLaserBeam(board, position, direction)\`. Show how an aggressive personality might overvalue active laser path intersections, while a calculated personality scales heuristic values by aggregating probabilistic data from \`getPieceThreatLevels(board)\`.
* **Defender Utility:** Utilize \`generateThreatMap(board)\` and \`computeSafetySteps(board, startR, startC, threatMap)\`. Show how to calculate Breadth-First Search (BFS) shortest-path metrics to safe zones, applying dynamic penalty weights for exposed units.

3. **\`getChallengeAction(board, gameState, playerColor)\`**
Model the unique challenge mechanic (a toss by one player and a 1d6 roll by both). 
* Implement a probabilistic risk-evaluation matrix that calculates the expected value (EV) of declaring a wager under these specific stochastic conditions.
* Provide an experimental bluffing subroutine that deviates from pure mathematical EV to simulate psychological gameplay when capturing high-utility nodes (like the 50-point piece).

**Required API Subroutines:**
Integrate standard 'LazerAI' global helper structures: \`getBoardState\`, \`getPossibleActions\`, \`applyLightweightAction\`, \`traceLaserBeam\`, \`generateThreatMap\`, \`getPieceThreatLevels\`, and \`computeSafetySteps\`.

**Output Constraints:**
Provide the final output as clean, production-grade JavaScript. Include explicit, pedagogical inline comments detailing the algorithmic time/space complexities (e.g., O(b^d)), the EV math behind the challenge dice/toss mechanics, and how students can mathematically tweak the heuristic weights to mutate the bot's underlying personality.`;

export default function BotDeveloperHub({ onBack, onStartSpectate, customBoards = [], onImportBoard, subMode = 'bot', onSubModeChange, activeTab = 'contract', onTabChange }) {
  ...
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border-color)', marginBottom: '24px', gap: '8px', flexWrap: 'wrap', width: '100%' }}>
            <button 
        ...
        {activeTab === 'contract' && (
          <div style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', lineHeight: '1.6', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            {/* LLM Prompt Assistant Panel */}
            <div className="glass-panel" style={{ padding: '24px', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '14px', position: 'relative' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                <h3 style={{ color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '8px', margin: 0, fontSize: '1.05rem' }}>
                  <Wand2 size={18} style={{ color: 'var(--neon-blue)' }} /> Using an LLM? Use this prompt to generate your bot!
                </h3>
                <button
                  className="cyber-button"
                  onClick={() => {
                    navigator.clipboard.writeText(LLM_PROMPT_CONTENT);
                  ...
                  style={{
                    ...
                    alignItems: 'center',
                    gap: '6px',
                    borderColor: promptCopied ? '#39ff14' : 'var(--neon-blue)',
                    color: promptCopied ? '#39ff14' : 'var(--neon-blue)'
                  }}
                >
                  {promptCopied ? <CheckCircle size={14} /> : <Download size={14} />} {promptCopied ? 'COPIED!' : 'COPY PROMPT'}
                </button>
              </div>
              <p style={{ margin: 0, fontSize: '0.88rem', color: 'var(--text-secondary)', lineHeight: '1.5' }}>
                You can copy the tailored prompt below and paste it into Gemini, ChatGPT, or Claude to quickly generate a structured, fully compatible custom Lazer bot skeleton.
              </p>
              <pre style={{
                maxHeight: '280px',
                overflowY: 'auto',
        ...
        {/* TAB: Headless Simulator (Fast Tournament) */}
        {activeTab === 'simulator' && (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', width: '100%', maxWidth: '550px', marginBottom: '15px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', width: '100%' }}>
                ...
                <button 
                  ...
                  style={{ flex: '1 1 auto', padding: '12px 24px', minHeight: '48px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', borderColor: 'var(--neon-blue)', color: 'var(--neon-blue)', fontWeight: 'bold' }}
                >
